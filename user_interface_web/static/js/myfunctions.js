...
function ScaleXForward(x) {
    return ((-scale * x) + offset_x + canvas_offset_x);
}

function ScaleYForward(y) {
    return ((scale * y) + offset_y + canvas_offset_y);
}

function ScaleXBack(x) {
    return (x - offset_x - canvas_offset_x) / (-1 * scale);
}

function ScaleYBack(y) {
    return (y - offset_y - canvas_offset_y) / scale;
}

function ScaleForward(x, y) {
    return (new Position(ScaleXForward(x), ScaleYForward(y)));
}

function ScaleBackward(x, y) {
    return (new Position(ScaleXBack(x), ScaleYBack(y)));
}
...
function drawBox(o, x, y, rot, init) {
    ...
    if (init === true) {
        ...
        box.transform("");
        box.attr({
            x: width - x_pos - (b_width - 2 * stroke),
            y: y_pos,
            transform: "r" + (-deg)
        });
    }
